...
import { defineProofContract, verifyClaim } from "@jev-kit/evidence-check";
import { parseEvidenceCheckInput } from "./index.js";

async function main(): Promise<void> {
  const input = parseEvidenceCheckInput(JSON.parse(await readStdin()) as unknown);
  const client = new JevClient({ ...(input.model === undefined ? {} : { defaultModel: input.model }) });
  const contract = input.contract === undefined ? undefined : defineProofContract(input.contract);
...
main().catch((error: unknown) => {
  ...
  process.stderr.write(`jev-kit-evidence-check: ${message}\n`);
  process.exitCode = 2;
});
